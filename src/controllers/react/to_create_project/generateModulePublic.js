...
const createRoutes = async (fullPath) => {
  const routesDir = path.join(fullPath, 'src', 'modules', 'public', 'routes');
  const filePath  = path.join(routesDir, 'PublicRoutes.jsx');
...
const createHome = async (fullPath) => {
  const pagesDir = path.join(fullPath, 'src', 'modules', 'public', 'pages');
  const filePath = path.join(pagesDir, 'HomePage.jsx');

  createFolder(pagesDir);

  const content = `import { PublicLayout } from "../../../layouts/public/PublicLayout";
...
const createContact = async (fullPath) => {
  const pagesDir = path.join(fullPath, 'src', 'modules', 'public', 'pages');
  const filePath = path.join(pagesDir, 'ContactPage.jsx');
  ...
  return (
    <PublicLayout>
      <main className="mt-50">
        ...
        {/* ... (rest of sections: logo cloud, team, blog) */}
      </main>
    </PublicLayout>
  );
};
